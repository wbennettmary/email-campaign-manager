    ...
    try:
        ...
        if accounts and not get_zoho_bounce_detector():
            ...
            if first_account:
                ...
                print("âš ï¸ No valid account found for bounce detector initialization")
            
            # Start background bounce monitoring
            start_bounce_monitoring(interval_seconds=300)  # Check every 5 minutes
            
            print("âœ… Zoho bounce detector initialized with account data")
            ...
            print("âš ï¸ No accounts available or bounce detector already initialized")
            
    except Exception as e:
        print(f"âš ï¸ Could not initialize Zoho bounce detector: {e}")
...
def get_campaign_logs(campaign_id):
    ...
    try:
        ...
        print(f"ðŸ“‹ Getting logs for campaign {campaign_id} (str: {campaign_id_str}, int: {campaign_id_int}), found {len(logs)} logs")
        return logs
    except Exception as e:
        print(f"âŒ Error reading campaign logs for {campaign_id}: {str(e)}")
        return []
...
def log_email(recipient, subject, sender, status, campaign_id=None, details=None):
    ...
    try:
        ...
        print(f"âŒ Error writing to email log: {e}")
...
def add_bounce_email(email, campaign_id, reason, subject=None, sender=None):
    ...
    try:
        ...
        if email not in existing_emails:
            ...
            print(f"ðŸ“§ Added {email} to bounce list for campaign {campaign_id}")
...
def add_delivered_email(email, campaign_id, subject=None, sender=None, details=None):
    ...
    try:
        ...
        if email not in existing_emails:
            ...
            print(f"âœ… Added {email} to delivered list for campaign {campaign_id}")
...
def filter_bounced_emails(emails, campaign_id):
    ...
    try:
        ...
        if removed_count > 0:
            print(f"ðŸš« Filtered out {removed_count} bounced emails from campaign {campaign_id}")
...
def dashboard():
    ...
    try:
        ...
        print(f"âš ï¸ Could not get Zoho bounce stats: {e}")
...
def api_accounts():
    if request.method == 'GET':
        ...
        try:
            ...
            print(f"ðŸ” Creating account with data: {json.dumps(data, indent=2)}")
            print(f"ðŸ‘¤ Current user: {current_user.username} (ID: {current_user.id}, Role: {current_user.role})")
            ...
            try:
                ...
                if not isinstance(accounts, list):
                    accounts = []
                print(f"ðŸ“Š Loaded {len(accounts)} existing accounts")
            except (FileNotFoundError, json.JSONDecodeError) as e:
                print(f"âš ï¸ No existing accounts file or invalid JSON: {e}")
                accounts = []
            
            # Generate new account ID
            new_id = max([acc['id'] for acc in accounts], default=0) + 1 if accounts else 1
            print(f"ðŸ†” Generated new account ID: {new_id}")
            ...
            print(f"ðŸ“ New account data: {json.dumps(new_account, indent=2)}")
            ...
            try:
                ...
                print(f"âœ… Successfully saved account to {ACCOUNTS_FILE}")
            except PermissionError as e:
                print(f"âŒ Permission error saving account: {e}")
                error_msg = f"Permission denied: Cannot write to {ACCOUNTS_FILE}. Please check file permissions on the server."
                return jsonify({'error': error_msg}), 500
            except Exception as e:
                print(f"âŒ Error saving to file: {e}")
            ...
            try:
                ...
                print(f"âœ… Added success notification")
            except Exception as e:
                print(f"âš ï¸ Could not add notification: {e}")
            
            print(f"ðŸŽ‰ Account creation completed successfully")
            return jsonify(new_account)
            
        except Exception as e:
            print(f"âŒ Error saving account: {str(e)}")
            import traceback
            traceback.print_exc()
...
def api_account(account_id):
    try:
        print(f"ðŸ” Processing account request: ID={account_id}, Method={request.method}")
        print(f"ðŸ‘¤ Current user: {current_user.username} (ID: {current_user.id}, Role: {current_user.role})")
        ...
        if not isinstance(accounts, list):
            accounts = []
        print(f"ðŸ“Š Loaded {len(accounts)} accounts from file")
    except (FileNotFoundError, json.JSONDecodeError) as e:
        print(f"âš ï¸ Error loading accounts file: {e}")
        accounts = []
    
    account = next((acc for acc in accounts if acc['id'] == account_id), None)
    
    if not account:
        print(f"âŒ Account {account_id} not found")
        return jsonify({'error': f'Account {account_id} not found'}), 404
    
    print(f"âœ… Found account: {account['name']} (ID: {account['id']}, Created by: {account.get('created_by', 'Unknown')})")
    ...
    if current_user.role != 'admin' and not has_permission(current_user, 'manage_accounts') and account.get('created_by') != current_user.id:
        print(f"âŒ Access denied: User {current_user.username} cannot access account {account_id}")
    ...
    if request.method == 'GET':
        ...
        try:
            with open(ACCOUNTS_FILE, 'w') as f:
                json.dump(accounts, f, indent=2)
            print(f"âœ… Successfully updated account {account_id}")
        except PermissionError as e:
            print(f"âŒ Permission error saving updated account: {e}")
            error_msg = f"Permission denied: Cannot write to {ACCOUNTS_FILE}. Please check file permissions on the server."
            return jsonify({'error': error_msg}), 500
        except Exception as e:
            print(f"âŒ Error saving updated account: {e}")
        ...
        print(f"ðŸ—‘ï¸ Attempting to delete account {account_id}")
        
        # Check if user has permission to delete accounts
        # Admin can delete any account, users can only delete their own
        if current_user.role != 'admin' and not has_permission(current_user, 'manage_accounts') and account.get('created_by') != current_user.id:
            print(f"âŒ Access denied: User {current_user.username} cannot delete account {account_id}")
            return jsonify({'error': 'Access denied. You can only delete your own accounts.'}), 403
        
        account_name = account['name']
        print(f"ðŸ“ Removing account '{account_name}' from list")
        
        try:
            accounts.remove(account)
            print(f"âœ… Account removed from list, {len(accounts)} accounts remaining")
        except ValueError as e:
            print(f"âŒ Error removing account from list: {e}")
            return jsonify({'error': f'Failed to remove account from list: {str(e)}'}), 500
        
        try:
            with open(ACCOUNTS_FILE, 'w') as f:
                json.dump(accounts, f, indent=2)
            print(f"âœ… Successfully saved updated accounts to file")
        except PermissionError as e:
            print(f"âŒ Permission error saving accounts file: {e}")
            error_msg = f"Permission denied: Cannot write to {ACCOUNTS_FILE}. Please check file permissions on the server."
            return jsonify({'error': error_msg}), 500
        except Exception as e:
            print(f"âŒ Error saving accounts file: {e}")
        ...
        try:
            ...
            print(f"âœ… Added deletion notification")
        except Exception as e:
            print(f"âš ï¸ Could not add notification: {e}")
        
        print(f"ðŸŽ‰ Account '{account_name}' deleted successfully")
...
def api_campaigns():
    if request.method == 'GET':
        ...
        try:
            print(f"ðŸ” Creating campaign - Memory usage: {log_memory_usage():.1f} MB")
            ...
            if write_json_file_simple(CAMPAIGNS_FILE, campaigns):
                ...
                print(f"âœ… Campaign created successfully - Memory usage: {log_memory_usage():.1f} MB")
                return jsonify(new_campaign)
            else:
                return jsonify({'error': 'Failed to save campaign to file'}), 500
                
        except Exception as e:
            print(f"âŒ Error creating campaign: {str(e)}")
            import traceback
            traceback.print_exc()
            cleanup_memory()
            return jsonify({'error': f'Internal server error: {str(e)}'}), 500
...
def get_campaign_logs_api(campaign_id):
    ...
    try:
        logs = get_campaign_logs(campaign_id)
        print(f"ðŸ” API: Getting logs for campaign {campaign_id}, found {len(logs)} logs")
        return jsonify(logs)
    except Exception as e:
        print(f"âŒ API Error getting logs for campaign {campaign_id}: {str(e)}")
        return jsonify([])
...
def delete_bounce(campaign_id, email):
    ...
    try:
        ...
        print(f"ðŸ” Attempting to delete bounce: campaign_id={campaign_id}, email={email}")
        ...
        print(f"ðŸ“Š Current bounce data: {json.dumps(bounce_data, indent=2)}")
        
        # Find and remove the specific bounce
        campaign_key = str(campaign_id)
        if campaign_key in bounce_data:
            print(f"âœ… Found campaign {campaign_id} with {len(bounce_data[campaign_key])} bounces")
            
            # Check if email exists in this campaign
            existing_emails = [bounce['email'] for bounce in bounce_data[campaign_key]]
            print(f"ðŸ“§ Emails in campaign {campaign_id}: {existing_emails}")
            ...
            if email in existing_emails:
                ...
                print(f"ðŸ—‘ï¸ Successfully deleted bounced email: {email} from campaign {campaign_id}")
                return jsonify({
                    'success': True, 
                ...
                print(f"âŒ Email {email} not found in campaign {campaign_id}")
                return jsonify({
                    'success': False, 
                    'message': f'Bounced email {email} not found in campaign {campaign_id}. Available emails: {existing_emails}'
                }), 404
        else:
            print(f"âŒ Campaign {campaign_id} not found in bounce data")
        ...
        print(f"âŒ Error deleting bounced email: {str(e)}")
        return jsonify({
            'success': False, 
...
def delete_campaign_bounces(campaign_id):
    ...
    try:
        ...
        if campaign_key in bounce_data:
            ...
            print(f"ðŸ—‘ï¸ Deleted all {deleted_count} bounced emails from campaign {campaign_id}")
            return jsonify({
                'success': True, 
        ...
        print(f"âŒ Error deleting campaign bounces: {str(e)}")
        return jsonify({
            'success': False, 
...
def delete_all_bounces():
    ...
    try:
        ...
        print(f"ðŸ—‘ï¸ Deleted all {total_deleted} bounced emails from all campaigns")
        return jsonify({
            'success': True, 
        ...
        print(f"âŒ Error deleting all bounces: {str(e)}")
        return jsonify({
            'success': False, 
    ...
    try:
        ...
        if detector:
            ...
            if bounce_info.get('email'):
                ...
                print(f"ðŸ“§ REAL ZOHO BOUNCE: {bounce_info['email']} - {bounce_info.get('bounce_reason', 'Unknown')}")
        ...
        print(f"âŒ Error processing Zoho bounce webhook: {str(e)}")
...
def template_manager():
    ...
    try:
        accounts = get_user_accounts(current_user)
        return render_template('template_manager.html', accounts=accounts)
    except Exception as e:
        flash(f'Error loading template manager: {str(e)}', 'error')
        return render_template('template_manager.html', accounts=[])
...
def send_campaign_emails(campaign, account):
    """
    Send emails for a campaign using Zoho CRM API
    """
    try:
        print(f"ðŸš€ Starting campaign: {campaign['name']}")
        ...
        if campaign_index is not None:
            campaigns[campaign_index]['status'] = 'running'
            campaigns[campaign_index]['started_at'] = datetime.now().isoformat()
        ...
        for i in range(0, total_emails, batch_size):
            ...
            for email in batch:
                try:
                    ...
                    success = send_single_email_via_zoho(account, template_id, email, subject)
                    
                    if success:
                        sent_count += 1
                        add_campaign_log(campaign_id, f"âœ… Email sent successfully to {email}")
                        log_email(email, subject, account['email'], 'sent', campaign_id)
                    else:
                        failed_count += 1
                        add_campaign_log(campaign_id, f"âŒ Failed to send email to {email}")
                        log_email(email, subject, account['email'], 'failed', campaign_id)
                    ...
                    add_campaign_log(campaign_id, f"âŒ {error_msg}")
                    log_email(email, subject, account['email'], 'error', campaign_id, str(e))
                    print(f"âŒ {error_msg}")
            
            # Update progress
            progress = min(100, ((i + batch_size) / total_emails) * 100)
            add_campaign_log(campaign_id, f"ðŸ“Š Progress: {progress:.1f}% ({i + batch_size}/{total_emails})")
        
        # Update final campaign status
        if campaign_index is not None:
            campaigns[campaign_index]['status'] = 'completed'
            campaigns[campaign_index]['completed_at'] = datetime.now().isoformat()
            campaigns[campaign_index]['sent_count'] = sent_count
            campaigns[campaign_index]['failed_count'] = failed_count
            write_json_file_simple(CAMPAIGNS_FILE, campaigns)
        
        # Add final summary
        summary = f"ðŸŽ‰ Campaign completed! Sent: {sent_count}, Failed: {failed_count}"
        add_campaign_log(campaign_id, summary)
        add_notification(summary, 'success', campaign_id)
        
        print(f"âœ… Campaign completed: {sent_count} sent, {failed_count} failed")
        ...
        if campaign_index is not None:
            ...
            campaigns[campaign_index]['error'] = str(e)
            write_json_file_simple(CAMPAIGNS_FILE, campaigns)

def send_single_email_via_zoho(account, template_id, recipient_email, subject):
    """
    Send a single email using Zoho CRM API
    """
    try:
        # Zoho API endpoint - using the working endpoint from your curl
        url = "https://crm.zoho.com/crm/v7/settings/functions/send_email_template2/actions/test"
        ...
        if response.status_code == 200:
            ...
            if result.get('status') == 'success':
                ...
                print(f"âŒ API Error (200): {result}")
                return False
        else:
            print(f"âŒ HTTP Error ({response.status_code}): {response.text}")
            return False
            
    except Exception as e:
        print(f"âŒ Exception sending email to {recipient_email}: {str(e)}")
        return False
...
def api_user(user_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            print(f"âŒ Permission error saving user update: {e}")
            error_msg = f"Permission denied: Cannot write to {USERS_FILE}. Please check file permissions on the server."
            return jsonify({'error': error_msg}), 500
        except Exception as e:
            print(f"âŒ Error saving user update: {e}")
        ...
        try:
            ...
            print(f"âŒ Permission error saving user deletion: {e}")
            error_msg = f"Permission denied: Cannot write to {USERS_FILE}. Please check file permissions on the server."
            return jsonify({'error': error_msg}), 500
        except Exception as e:
            print(f"âŒ Error saving user deletion: {e}")
...
def log_memory_usage():
    ...
    try:
        ...
        print(f"ðŸ“Š Memory Usage: {memory_info.rss / 1024 / 1024:.1f} MB")
...
def cleanup_memory():
    ...
    try:
        gc.collect()
        print(f"ðŸ§¹ Memory cleanup completed. Current usage: {log_memory_usage():.1f} MB")
    except:
        pass
...
def write_json_file_simple(filename, data):
    ...
    try:
        ...
        print(f"âŒ Error writing {filename}: {e}")
        return False
